...
//		System.out.println(stack.pop());
	...
	public void sortStackByStack(Stack<Integer> stack) {
		Stack<Integer> help = new Stack<Integer>();//辅助的栈help
		while (!stack.isEmpty()) {
			int cur = stack.pop();//弹出stack中的当前元素 Stack 是LIFO（先进后出）
			while (!help.isEmpty() && (cur > help.peek())) {//cur大于help的栈顶元素，弹出help的栈顶元素给stack
				stack.push(help.pop());
			}
			help.push(cur);//cur小于等于help的栈顶元素时，help压入cur
		}
		while (!help.isEmpty()) {//排序好，反向压回来 ,完成最终排序
			stack.push(help.pop());
		}
		System.out.println(new Gson().toJson(stack));
